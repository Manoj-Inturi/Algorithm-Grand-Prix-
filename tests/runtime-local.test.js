...
async function main() {
    ...
    try {
        ...
        assert.ok(qr.json.url.endsWith("/controller"), "QR target must always be the controller route");
        assert.ok(qr.json.url.endsWith("/controller"));
        assert.ok(qr.json.url.includes(":3000"));
        assert.ok(qr.json.qr.startsWith("QR:"));
        ...
        assert.strictEqual(round.json.state.players[0].name, "Manoj");
        assert.strictEqual(round.json.state.players[1].name, "Ravi");
        assert.strictEqual(round.json.state.players[0].algorithm, null);
        ...
        assert.strictEqual(vercelQr.json.url, "https://algorithm-grand-prix.example/controller");
