...
const controllerRound = document.getElementById("controller-round");
const controllerMazeName = document.getElementById("controller-maze-name");
const controllerMazeSize = document.getElementById("controller-maze-size");
const controllerOpenCells = document.getElementById("controller-open-cells");
const controllerPlayerCount = document.getElementById("controller-player-count");
const controllerPlayersList = document.getElementById("controller-players-list");
...
let controllerRoundClosed = false;
let lastCelebrationKey = "";

if (new URLSearchParams(window.location.search).get("join") === "new") {
    const clearFreshName = () => clearControllerNameUI();
    window.addEventListener("load", clearFreshName, { once: true });
    window.setTimeout(clearFreshName, 250);
}

function clearControllerNameUI() {
    playerName = "";
    if (playerNameInput) {
    ...
    if (nameState) nameState.textContent = "REQUIRED";
    if (nameMessage) {
        nameMessage.textContent = "Enter your name before choosing an algorithm.";
...
const MAX_WALLS = 2;
const MAX_TERRAIN = 2;
const PLAYER_COLORS = { 1: "#E53935", 2: "#1E88E5", 3: "#43A047", 4: "#FB8C00" };

function updateControllerGameInfo(players) {
    const list = Array.isArray(players) ? players : [];
    if (controllerRound) controllerRound.textContent = String(currentRound || 1).padStart(2, "0");
    if (controllerMazeName) controllerMazeName.textContent = mazeName?.textContent?.replace(/^MAZE\s+\d+\s*•\s*/i, "") || (currentMaze ? "CURRENT MAZE" : "WAITING");
    if (controllerMazeSize) controllerMazeSize.textContent = currentMaze ? `${currentMaze.length} × ${currentMaze[0]?.length || 0}` : "71 × 71";
    if (controllerOpenCells) {
        const open = currentMaze ? currentMaze.reduce((sum, row) => sum + row.filter(cell => cell === 0).length, 0) : 0;
        controllerOpenCells.textContent = open ? open.toLocaleString() : "—";
    }
    if (controllerPlayerCount) controllerPlayerCount.textContent = `${list.length} / 4`;
    if (controllerPlayersList) {
        if (!list.length) {
            controllerPlayersList.innerHTML = '<span>Waiting for players.</span>';
        } else {
            controllerPlayersList.innerHTML = list.slice().sort((a,b)=>Number(a.number)-Number(b.number)).map(player => {
                ...
                const status = player.ready ? "READY" : "SETUP";
                const me = Number(player.number) === Number(playerNumber) ? " • YOU" : "";
                return `<div class="controller-player-row"><span style="color:${color}">P${player.number}</span><strong>${escapeHtml(name)}</strong><span>${escapeHtml(algorithm)}</span><span>${status}${player.isBot ? " • BOT" : me}</span></div>`;
            }).join("");
        }
    }
}

function updatePlayersNeeded(players) {
    ...
    if (connected < 4) {
        ...
        playersNeeded.textContent = `${missing} MORE PLAYER${missing === 1 ? "" : "S"} NEEDED TO START`;
        return;
    }

    if (ready < 4) {
        const missingReady = 4 - ready;
        playersNeeded.textContent = `${missingReady} PLAYER${missingReady === 1 ? "" : "S"} STILL NEED TO READY`;
        return;
    }

    playersNeeded.textContent = "ALL PLAYERS READY • STARTING RACE";
}

function lockControllerForRejoin(message = "ROUND RESET • SCAN THE MAIN SCREEN QR CODE TO JOIN AGAIN") {
    controllerRoundClosed = true;
    readyButton.disabled = true;
    algorithmButtons.forEach(button => button.disabled = true);
    predictionButtons.forEach(button => button.disabled = true);
    if (playerNameInput) playerNameInput.disabled = true;
    if (saveNameButton) saveNameButton.disabled = true;
    if (playersNeeded) playersNeeded.textContent = message;
    RaceAudio.warning();
    if (connectionStatus) connectionStatus.textContent = "SESSION CLOSED • SCAN QR TO REJOIN";
    if (mazeMessage) {
        mazeMessage.textContent = "NEW ROUND • SCAN THE MAIN SCREEN QR CODE TO CONNECT AGAIN";
...
function friendlyConnectionError(error) {
    ...
    if (code === "ROUND_REJOIN_REQUIRED") return "ROUND RESET • SCAN THE MAIN SCREEN QR AGAIN";
...
socket.on("player_assigned", data => {
    ...
    if (Array.isArray(data?.players)) { updateControllerGameInfo(data.players); updatePlayersNeeded(data.players); }

    if (data?.nameError) {
        nameSavePending = false;
        saveNameButton.disabled = false;
        playerNameInput.disabled = false;
        if (data.nameError.code === "NAME_CONFLICT") {
            nameMessage.textContent = data.nameError.message || "That name is already in this round.";
            nameMessage.className = "name-message error";
        } else {
            nameMessage.textContent = data.nameError.message || "Enter a valid name.";
            nameMessage.className = "name-message error";
        }
        subtitle.textContent = `PLAYER ${playerNumber} • ENTER YOUR NAME`;
...
socket.on("players_updated", players => {
    updateControllerGameInfo(players);
    updatePlayersNeeded(players);
...
socket.on("maze_selected", mazeMap => {
    if (controllerRoundClosed) return;
    if (!mazeMap) return;
...
socket.on("round_reset", payload => {
    ...
    resetMobileResults();
    lastCelebrationKey = "";
    clearControllerNameUI();
    reconnectingWithSavedName = false;
    lockControllerForRejoin("ROUND RESET • SCAN THE MAIN SCREEN QR CODE TO JOIN AGAIN");
    console.log("[GP] ROUND RESET", currentRound);
});

socket.on("round_rejoin_required", data => {
    lockControllerForRejoin(data?.message || "ROUND RESET • SCAN THE MAIN SCREEN QR CODE TO JOIN AGAIN");
});
...
readyButton.addEventListener("click", () => {
    ...
    socket.timeout(7000).emit("player_ready", {
        ...
        if (error || !response?.ok) {
            ...
            readyButton.textContent = "READY TO RACE";
        ...
        algorithmButtons.forEach(button => button.disabled = true);
        predictionButtons.forEach(button => button.disabled = true);
        updatePlayersNeeded(response.state?.players || []);
...
function vibrateResults(place) {
    try {
        if (!navigator.vibrate) return;
        const pattern = Number(place) === 1
            ? [90, 60, 150, 60, 240]
            : Number(place) === 2
                ? [80, 50, 140]
                : [70, 45, 90];
        navigator.vibrate(pattern);
        window.setTimeout(() => { try { navigator.vibrate([55, 35, 110]); } catch (_) {} }, Number(place) === 1 ? 420 : 260);
    } catch (_) {}
}
...
function renderMobileResults(results) {
    ...
    mobileRanking.innerHTML = sorted.map(item => {
        ...
        const rankColor = PLAYER_COLORS[item.place] || "#fff";
        return `<div class="mobile-rank-row" style="border-left:3px solid ${rankColor}">
            <div class="place" style="color:${rankColor}">${trophy}</div>
            <div>
    ...
    const resultKey = `${mine.round || currentRound}:${mine.playerNumber}:${mine.place}`;
    mobileCelebration.classList.remove("hidden");
    if (lastCelebrationKey !== resultKey) {
        lastCelebrationKey = resultKey;
        vibrateResults(mine.place);
        RaceAudio.report();
        window.setTimeout(() => {
