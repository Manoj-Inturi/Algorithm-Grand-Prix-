...
const HEARTBEAT_WRITE_INTERVAL_MS = 3_000;
const STATE_VERSION = "v8";
...
const MAX_INVALIDATED_TOKENS = 60;
let localState = null;

app.use(express.json({ limit: "100kb" }));

// This app displays QR codes; it does not need camera or microphone access.
...
function publicState(state) {
    const connectedPlayers = state.players.length;
    const readyPlayers = state.players.filter(player => player.ready).length;
    const playersNeededToStart = Math.max(0, PLAYER_LIMIT - connectedPlayers);
    const readyPlayersNeededToStart = Math.max(0, PLAYER_LIMIT - readyPlayers);
...
function cleanupExpiredPlayers(state) {
    ...
    state.players = state.players.filter(player =>
        player.isBot || Number(player.lastSeen || 0) >= cutoff
    );
...
function resetRoundState(state) {
    ...
    const retiredTokens = previousPlayers
        .filter(player => !player.isBot && player.token)
        .map(player => player.token);

    for (const player of previousPlayers) resetPlayerForRound(player);

    state.invalidatedTokens = Array.from(new Set([
        ...(Array.isArray(state.invalidatedTokens) ? state.invalidatedTokens : []),
        ...retiredTokens
    ])).slice(-MAX_INVALIDATED_TOKENS);
...
function buildBotName(algorithm) {
    return `BOT • ${algorithm}`;
}

function botCandidateCells(state, algorithm) {
    const candidates = [];
    const phase = algorithm.length * 17 + state.roundNumber * 31;
    for (let row = 2; row < MAZE_SIZE - 2; row++) {
        for (let col = 2; col < MAZE_SIZE - 2; col++) {
            if (state.currentMaze?.[row]?.[col] !== 0) continue;
            if ((row + col + phase) % 7 !== 0) continue;
            candidates.push({ row, col });
        }
    }
    // Keep the list deterministic but spread selections around the maze.
    candidates.sort((a, b) => {
        const sa = ((a.row * 97 + a.col * 53 + phase) % 1000);
        const sb = ((b.row * 97 + b.col * 53 + phase) % 1000);
        return sa - sb || a.row - b.row || a.col - b.col;
    });
    return candidates;
}

function chooseBotLoadout(state, bot) {
    if (!bot?.isBot || !bot.algorithm) return;
    const candidates = botCandidateCells(state, bot.algorithm);
    const used = new Set();

    if (bot.algorithm === "DIJKSTRA") {
        for (const candidate of candidates) {
            const key = `${candidate.row},${candidate.col}`;
            if (used.has(key) || cellClaimedByAnotherPlayer(state, candidate.row, candidate.col, bot.token)) continue;
            bot.terrain.push(candidate);
            used.add(key);
            if (bot.terrain.length >= 2) break;
        }
        gpLog("info", "BOT_LOADOUT", {
            algorithm: bot.algorithm,
            sand: bot.terrain.length,
            round: state.roundNumber
        });
        return;
    }
    ...
    for (const candidate of candidates) {
        const key = `${candidate.row},${candidate.col}`;
        if (used.has(key) || cellClaimedByAnotherPlayer(state, candidate.row, candidate.col, bot.token)) continue;
        if (!safeWall(state, bot, candidate, bot.walls)) continue;
        bot.walls.push(candidate);
        used.add(key);
        if (bot.walls.length >= 2) break;
    }

    gpLog("info", "BOT_LOADOUT", {
        algorithm: bot.algorithm,
        walls: bot.walls.length,
        round: state.roundNumber
    });
}

function chooseBotPrediction(state, bot) {
    const previousWinner = state.lastRaceResults?.order?.[0];
    if (ALGORITHMS.includes(previousWinner)) return previousWinner;

    const firstRoundGuess = {
        BFS: "BFS",
        DFS: "A*",
        DIJKSTRA: "DIJKSTRA",
        "A*": "A*"
    };
    return firstRoundGuess[bot.algorithm] || "A*";
}

function fillBots(state) {
    const realPlayers = state.players.filter(player => !player.isBot);

    if (realPlayers.some(player => !player.algorithm || !ALGORITHMS.includes(player.algorithm))) {
        throw buildError(409, "BOTS_NEED_ALGORITHMS", "Every real player must select an algorithm before bots can fill the remaining slots.");
    }

    if (state.players.length >= PLAYER_LIMIT) {
        return { added: 0, totalBots: state.players.filter(player => player.isBot).length };
    }

    const usedAlgorithms = new Set(state.players.map(player => player.algorithm).filter(Boolean));
    const missingAlgorithms = ALGORITHMS.filter(algorithm => !usedAlgorithms.has(algorithm));
    const slots = PLAYER_LIMIT - state.players.length;
    const algorithmsToAdd = missingAlgorithms.slice(0, slots);

    for (const algorithm of algorithmsToAdd) {
        const bot = {
            token: `bot:${state.roundNumber}:${algorithm.replace(/[^a-zA-Z0-9]/g, "")}`,
            number: state.players.reduce((max, player) => Math.max(max, Number(player.number) || 0), 0) + 1,
            name: buildBotName(algorithm),
            algorithm,
            ready: true,
            walls: [],
            terrain: [],
        ...
        chooseBotLoadout(state, bot);
        bot.prediction = chooseBotPrediction(state, bot);
        gpLog("info", "BOT_ADDED", { algorithm, round: state.roundNumber, walls: bot.walls.length, sand: bot.terrain.length, prediction: bot.prediction });
    }

    return {
        added: algorithmsToAdd.length,
        algorithms: algorithmsToAdd,
        totalBots: state.players.filter(player => player.isBot).length
    };
}
...
function buildJoinUrlFromRequest(req) {
    ...
    return `${origin}/controller?join=new`;
}
...
app.post("/api/realtime/leave", async (req, res) => {
    try {
        assertSharedStateAvailable();
        ...
        const { result, state } = await mutateState(current => {
            if (role === "screen") {
                if (!current.screen || current.screen.token !== token) {
                    return { ok: true, removed: false, role, number: null };
                }
                current.screen = null;
                gpLog("info", "SCREEN_LEFT", {});
                return { ok: true, removed: true, role, number: null };
            }

            const index = current.players.findIndex(player => player.token === token);
            if (index < 0) return { ok: true, removed: false, role, number: null };

            const player = current.players[index];
            if (current.raceState === "racing") {
                throw buildError(409, "RACE_IN_PROGRESS", "You cannot leave while the race is in progress.");
            }

            current.players.splice(index, 1);
            gpLog("info", "PLAYER_LEFT", { player: player.number, name: player.name || "" });
            return { ok: true, removed: true, role, number: player.number };
        });

        res.set("Cache-Control", "no-store");
        res.json({ ...result, state: publicState(state) });
    } catch (error) {
        sendApiError(res, error);
    }
});
...
app.post("/api/realtime/join", async (req, res) => {
    try {
        ...
        const { result, state } = await mutateState(current => {
            ...
            if (role === "player" && Array.isArray(current.invalidatedTokens) && current.invalidatedTokens.includes(token)) {
                throw buildError(409, "ROUND_REJOIN_REQUIRED", "This round ended. Scan the main screen QR code to join the new round.");
            }
    ...
    try {
        ...
        const { result, state } = await mutateState(current => {
            ...
            switch (event) {
                case "fill_bots": {
                    if (role !== "screen") {
                        throw buildError(403, "SCREEN_ONLY", "Only the main screen can fill bot slots.");
                    }
                    if (current.raceState !== "waiting") {
                        throw buildError(409, "ROUND_LOCKED", "Bots can only be added while the round is waiting.");
                    }
                    const botResult = fillBots(current);
                    startRaceIfReady(current);
                    return { ok: true, ...botResult, raceState: current.raceState };
                }
